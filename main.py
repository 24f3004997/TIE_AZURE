...
# 3. App Pages (UPDATED TO MATCH YOUR BROWSER LINKS)
...
async def analyze_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        video_data = get_val(results[3], {"eye_contact_score": 0, "gesture_energy_score": 0, "distraction_events": 0})
        
        # --- NEW: Distraction Calculation ---
        raw_distractions = video_data.get("distraction_events", 0)
        focus_score = max(0, 100 - (raw_distractions * 5)) # Start at 100, lose 5 pts per distraction
        distraction_status = "High Risk" if raw_distractions > 5 else "Good Focus"
        
        content_data = get_val(results[0], {"clarity_score": 0, "wpm": 0, "filler_count": 0})

        content_data = get_val(results[0], {
        "clarity_score": 0, "wpm": 0, 
        "complexity_score": 0, "depth_score": 0, "topic_level": "Unknown"})

        vocal_data = get_val(results[1], {"avg_pitch": 0, "delivery_status": "Neutral", "emotion_detected": "Neutral"})
        ...
        data = {
            ...
            "distraction": {
                "events": raw_distractions,
                "focus_score": focus_score,
                "status": distraction_status},
            "speech": {
            "fillers": content_data.get("filler_count", 0),
            "status": "Clean" if content_data.get("filler_count", 0) < 5 else "Needs Polish"
        },
            
            "cognitive": {
            "complexity": content_data.get("complexity_score", 0),
            "depth": content_data.get("depth_score", 0),
            "level": content_data.get("topic_level", "Basic")
            },
            "sentiment": {
            "emotion": vocal_data.get("emotion_detected", "Neutral"),
            "intensity": vocal_data.get("emotional_intensity", 50),
            "status": "Good" if vocal_data.get("emotional_intensity", 0) > 60 else "Flat"
        }
        }
